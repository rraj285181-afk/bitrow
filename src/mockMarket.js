  ...
  async fetchHistoryFromApi(symbol, timeframeMin) {
    ...
    try {
      ...
      if (symbol === 'BTC/USDT' || symbol === 'ETH/USDT') {
        ...
        const intervalStr = this.mapTimeframeToHyperliquid(timeframeMin);
        const limit = 500;
        const startTime = Date.now() - limit * timeframeMin * 60 * 1000;
        ...
        const intervalStr = this.mapTimeframeToYahooInterval(timeframeMin);
